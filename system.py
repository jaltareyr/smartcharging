...
def current_assignment() :
    ...
    for i in range(len(cars)):
        ...
        powers[i] = power(cars[i][0]["user_id"])
        #except:
        #    pass
    ...
    for i in selected:
        if cars[i]!=[]:
            ...
            sql = "UPDATE current_values SET soct = %s WHERE user_id = %s"
            cursor.execute(sql,[int(SoC),user_id])
            t = cars[i][0]["departure_duration_smart"]
            sql = "UPDATE current_values SET departure_duration_smart = %s WHERE user_id = %s"
            cursor.execute(sql,[t-15,user_id])

            cursor.execute("SELECT * FROM current_values WHERE user_id = %s",[user_id,])
            crs = cursor.fetchall()

            if P>7500:
                updated_power = int(crs[0]["power"] + 7500)
            else:
                updated_power = int(crs[0]["power"] + P)

            cursor.execute("UPDATE current_values SET power = %s WHERE user_id = %s",[updated_power,user_id])
